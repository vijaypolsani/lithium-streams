...
import com.lithium.streams.compliance.service.KafkaConsumer;
...
public class KafkaConsumerImpl<K, V> implements KafkaConsumer<K, V> {
	...
	public PartitionMetadata findLeader(List<String> a_seedBrokers, int a_port, String a_topic, int a_partition) {
		...
		if (returnMetaData != null) {
			...
			for (kafka.cluster.Broker replica : returnMetaData.replicas()) {
				a_seedBrokers.add(replica.host());
			}
		}
	...
	@Override
	public TopicMetadataResponse send(TopicMetadataRequest topicMeta) {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public FetchResponse fetchAndProcess(FetchRequest fetchRequest) {
		// TODO Auto-generated method stub
		return null;
	}
	...
	public void process(long a_maxReads, String a_topic, int a_partition, List<String> a_seedBrokers, int a_port)
		...
		while (a_maxReads > 0) {
			...
			kafka.api.FetchRequest req = new FetchRequestBuilder().clientId(clientName).addFetch(a_topic, a_partition,
					readOffset, 100000) // Note: this fetchSize of 100000 might need to be increased if large batches are written to Kafka
					.build();
